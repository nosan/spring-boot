...
However, Spring MVC, Spring WebFlux, Spring WebSocket, Spring GraphQL, and JPA all require a bean named `applicationTaskExecutor`.
For Spring MVC, Spring WebFlux, and JPA, this bean must be of type javadoc:org.springframework.core.task.AsyncTaskExecutor[], whereas Spring GraphQL and Spring WebSocket do not enforce this type requirement.
...
However, this bean will not be used for Spring MVC, Spring WebFlux, Spring WebSocket, Spring GraphQL, JPA, and background initialization of beans.
====


If your application needs multiple `Executor` beans for different integrations, such as one for regular task execution with javadoc:org.springframework.scheduling.annotation.EnableAsync[format=annotation] and other for Spring MVC, Spring WebFlux, Spring WebSocket, Spring GraphQL, JPA, and background initialization of beans, you can configure them as follows.

include-code::multiple/MyTaskExecutorConfiguration[]

[WARNING]
====
Keep in mind that if you define a custom javadoc:java.util.concurrent.Executor[] bean with naming it `bootstrapExecutor`, it will be used for background initialization of beans instead of `applicationTaskExecutor`.
====
[TIP]
====
