...
class HazelcastServerConfiguration {
	...
	static class HazelcastServerConfigFileConfiguration {
		...
		HazelcastInstance hazelcastInstance(HazelcastProperties properties,
			...
			Config config = getConfig(properties, hazelcastConfigCustomizers);
			return new HazelcastInstanceFactory(config).getHazelcastInstance();

		}

		private static Config getConfig(HazelcastProperties properties,
				ObjectProvider<HazelcastConfigCustomizer> hazelcastConfigCustomizers) throws IOException {
			...
			if (config != null) {
				return new HazelcastConfigFactory(config, customizers).getConfig();
			}
			return new HazelcastConfigFactory(customizers).getConfig();
		}
	...
	static class ConfigAvailableCondition extends HazelcastConfigResourceCondition {
		...
		ConfigAvailableCondition() {
			super(CONFIG_SYSTEM_PROPERTY, "file:./hazelcast.xml", "classpath:/hazelcast.xml", "file:./hazelcast.yaml",
					"file:./hazelcast.yml", "classpath:/hazelcast.yaml", "classpath:/hazelcast.yml");
		}
